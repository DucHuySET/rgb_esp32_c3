...
void hanldeWhiteDim(String messageTemp){
  int colonIndex = messageTemp.indexOf(':');
  String numbersString = messageTemp.substring(colonIndex + 1);
  int numbers = numbersString.toInt();
  int dimensity = 256 * numbers / 100;
  analogWrite(WARM_WHITE, dimensity);
  Serial.print("dimensity: ");
  Serial.println(dimensity);
}
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "message/control/white") {
    ...
    if (messageTemp.indexOf("on") != -1){
      ...
      control_white = 1;
      Serial.println("white dim");
      send_message_mqtt("white","dim");
      hanldeWhiteDim(messageTemp);
    }
  }
